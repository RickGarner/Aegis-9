...
class Settings(BaseSettings):
    ...
    role_mapping_path: Path = Field(default=Path("config/role-mappings.json"), validation_alias="JARVIS_ROLE_MAPPING_PATH")
    ...
    freeflow_timeout_seconds: float = Field(default=30, ge=1, le=60, validation_alias="JARVIS_FREEFLOW_TIMEOUT_SECONDS")
    ...
    moveit_credential_target: str = Field(default="Aegis-9/MoveIT/ReadOnly", validation_alias="JARVIS_MOVEIT_CREDENTIAL_TARGET")
    ...
    alert_smtp_username: str | None = Field(default=None, validation_alias="JARVIS_ALERT_SMTP_USERNAME")
    alert_smtp_password: str | None = Field(default=None, validation_alias="JARVIS_ALERT_SMTP_PASSWORD")
    alert_smtp_credential_target: str = Field(default="Aegis-9/SMTP/Alerts", validation_alias="JARVIS_ALERT_SMTP_CREDENTIAL_TARGET")
    ...
    def resolve_provider_base_url(self) -> "Settings":
        ...
        if not self.role_mapping_path.is_absolute():
            self.role_mapping_path = (Path(__file__).resolve().parents[2] / self.role_mapping_path).resolve()
