...
Use the revision-scoped sandbox tools to maintain todos, inspect context, and create or edit implementation files through a change set. Every file change must be previewed, validated, and committed before you claim it was created. Use only discovered typed validation recipes; never request arbitrary shell commands. Record validation evidence in the todos. Also return the canonical primary workflow source in a fenced {language} block so A.E.G.I.S.-9 can hash and retain the immutable review artifact.

Produce the workflow and the test artifacts required by the separately approved test plans. Do not claim tests were executed unless retained tool evidence proves it. The result remains non-production until implementation review, retained test evidence, user acceptance, a bound schedule/conditions record, and independent supervisor approval are complete."""
