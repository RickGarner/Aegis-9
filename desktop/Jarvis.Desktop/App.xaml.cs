...
public partial class App : Application
{
    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        try
        {
            ...
            if (!ready)
            {
                ...
                if (!ready)
                {
                    MessageBox.Show("Jarvis could not start the local backend within the timeout. See backend/logs for details.", "Jarvis", MessageBoxButton.OK, MessageBoxImage.Warning);
                }
            }
        }
        catch (Exception ex)
        {
            MessageBox.Show($"Failed to start the local backend: {ex.Message}", "Jarvis", MessageBoxButton.OK, MessageBoxImage.Error);
        }
    ...
    private static (string BackendDirectory, string PythonExecutable) ResolveBackendRuntime()
    {
        ...
        throw new DirectoryNotFoundException("Could not locate the Jarvis backend directory.");
    }
