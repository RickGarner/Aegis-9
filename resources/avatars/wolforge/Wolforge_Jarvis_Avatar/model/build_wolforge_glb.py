...
def ellipsoid_mesh(name, size, material, segments=12, rings=6):
    """Create a faceted organic form for the wolf head and muzzle silhouette."""
    ...
    for ring in range(rings + 1):
        ...
        for segment in range(segments):
            ...
            vertices.append((rx * cos_phi * cos_theta, ry * sin_phi, rz * cos_phi * sin_theta))
            normal = (cos_phi * cos_theta / rx, sin_phi / ry, cos_phi * sin_theta / rz)
    ...
    for ring in range(rings):
        for segment in range(segments):
            ...
            faces.extend((current, next_ring, ring * segments + next_segment))
            faces.extend((ring * segments + next_segment, next_ring, (ring + 1) * segments + next_segment))
    pi = accessor(vertices); no = accessor(normals); ix = accessor([(i,) for i in faces], "SCALAR", 5123, 34963)
    meshes.append({"name": name, "primitives": [{"attributes": {"POSITION": pi, "NORMAL": no}, "indices": ix, "material": material}]})
    return len(meshes) - 1

def prism_mesh(name, size, material):
    """Create a tapered triangular ear instead of a rectangular ear block."""
    width, height, depth = [value / 2 for value in size]
    vertices = [(-width, -height, -depth), (width, -height, -depth), (0, height, -depth),
                (-width * .65, -height, depth), (width * .65, -height, depth), (0, height * .85, depth)]
    faces = [0, 1, 2, 3, 5, 4, 0, 3, 4, 0, 4, 1, 1, 4, 5, 1, 5, 2, 2, 5, 3, 2, 3, 0]
    pi = accessor(vertices); normals = []
    for vertex in vertices:
        length = math.sqrt(sum(value * value for value in vertex)) or 1
        normals.append(tuple(value / length for value in vertex))
...
neck=node("Neck",ellipsoid_mesh("NeckMesh",(2.35,1.45,1.25),NAVY),root,t=(0,-2.05,0))
head=node("Head",ellipsoid_mesh("Cranium",(2.65,2.65,1.55),GUN,16,8),neck,t=(0,1.75,.02))

# Ears are separate animation targets.
ear_mesh=prism_mesh("EarArmor",(.78,1.85,.62),SILVER)
...
brow=prism_mesh("BrowPlate",(1.28,.52,.28),GUN)
node("Brow.L",brow,head,t=(-.67,.42,-.91),r=qz(-.28)); node("Brow.R",brow,head,t=(.67,.42,-.91),r=qz(.28))
cheek=ellipsoid_mesh("CheekPlate",(1.05,1.15,.28),SILVER,8,3)
...
eye=ellipsoid_mesh("EyeLens",(.64,.20,.12),CYAN,8,3)
...
upper=node("UpperMuzzle",ellipsoid_mesh("UpperMuzzleMesh",(1.46,1.65,.92),SILVER,12,5),head,t=(0,-.78,-1.18),r=qx(-.06))
nose=node("Nose",ellipsoid_mesh("NoseMesh",(.78,.48,.38),BLACK,10,4),upper,t=(0,-.67,-.54))
mouth=node("MouthInterior",ellipsoid_mesh("MouthInteriorMesh",(1.08,.42,.5),DARK,10,4),head,t=(0,-1.53,-1.28))
jaw=node("Jaw",ellipsoid_mesh("LowerJawMesh",(1.14,.58,.7),SILVER,10,4),head,t=(0,-1.53,-1.18),extras={"control":"jaw","visemes":["rest","MBP","FV","A","E","I","O","U","L","SZ"]})
